...
model_path = r'h5_model\RCNN_crop_weed_classification_model.h5'
svm_model_path = r'h5_model\svm_classifier.pkl'

model = tf.keras.models.load_model(model_path)
model_without_last_two_fc = tf.keras.models.Model(model.inputs,model.layers[-5].output)

save_path = r'predicted_image/'
...
def detection(img_path,confidence=0.9,iou_thresh=0.1):
    ...
    for rect,score,cls in final:
        ...
        cv2.imshow('upload.html',imOut)

        # cv2.imwrite(os.path.join(str(save_path) + 'prediction.jpeg'.format(int(time.time()))),imOut)
        cv2.imwrite(f"predicted_image/{format(int(time.time()))}.png",imOut)
        
        # for n,_ in enumerate(os.listdir(save_path)):
        #     cv2.imwrite(f'predicted_image/image_{n}.png',imOut)
        cv2.waitKey(0)
    
    return final

if __name__=='__main__':
    detection(r'media\agri_0_39_HnbldkW.jpeg')